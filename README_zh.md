...
该项目按《`在线投票系统设计方案.md`》实现一个可本地演示的在线投票系统：

- **Web 前端**：`web/index.html`（原生 JS），展示投票项与票数，轮询刷新
- **HTTP 网关**：`cmd/httpserver`（Gin），接收浏览器 HTTP 请求并通过 gRPC 调用业务服务
- **业务服务**：`cmd/grpcserver`（gRPC），写入 Redis 并返回投票结果
- **数据存储**：Redis（按方案的 key 结构；关键写操作用 Lua 原子脚本）
...
- **`HTTP_ADDR`**：httpserver 监听地址（默认 `:8080`）
- **`GRPC_ADDR`**：\n+  - httpserver 连接的 grpcserver 地址（默认 `:9090`，建议显式填 `127.0.0.1:9090`）\n+  - grpcserver 自身监听地址（默认 `:9090`）\n+- **`REDIS_ADDR`**：Redis 地址；不设置则 grpcserver 使用内存存储（仅便于编译运行，不建议用于演示一致性）

### 主要 HTTP 接口（按方案）

说明：所有接口都需要请求头 `X-User-Id`（演示用）。
...
- **统计**：`GET /polls/get_polls_by_vote`
...
curl -X POST "http://localhost:8080/polls/createPoll" \
  ...
  -H "X-User-Id: user_1" \
  -d "{\"question\":\"你更喜欢哪种技术话题？\",\"options\":[\"Go 并发\",\"Kubernetes\",\"Redis\"],\"expires_at\":\"2030-01-01T00:00:00Z\"}"
```
